...
ספריית PHP מודרנית ומוקפדת לעבודה מול ה-API של [SMS4Free](https://www.sms4free.co.il/) — שליחת הודעות SMS,
טיפול במספרי טלפון ישראליים ויצירת קודי אימות חד-פעמיים.

**🇬🇧 [README in English](README.md)** · [תיעוד ה-API](docs/api-reference.he.md) · [שדרוג מגרסה 1.x](UPGRADING.md)

---

## למה בכלל צריך ספרייה

הקריאה ל-API של SMS4Free היא בסך הכול קריאת `curl` אחת. כל מה שמסביבה הוא החלק המעניין:

- **מספרי טלפון מגיעים מלוכלכים.** `054-123-4567`, `+972 54 123 4567` ו-`00972541234567` הם אותו קו בדיוק.
  כולם עוברים נרמול לצורה קנונית אחת, וכל מה שאינו מספר סלולרי ישראלי תקין נפסל *לפני* שנשלחת בקשה.
- **עברית שוברת טיפול נאיבי במחרוזות.** חיתוך הודעה עם `substr()` חותך תו דו-בייטי באמצע ומייצר ג׳יבריש,
  ו-`strlen()` סופר בייטים ולא תווים. כאן הכול בטוח למולטי-בייט.
- **צריך להבדיל בין סוגי כשלונות.** "המספר לא תקין", "הספק אומר שנגמרה היתרה" ו"הרשת נפלה" מחייבים שלוש
  תגובות שונות באפליקציה. לכל אחד מהם יש טיפוס Exception משלו.
- **פרטי ההתחברות הם סוד.** הם לא נכתבים לתוך הודעות שגיאה, והם מוסתרים גם מפלט של `var_dump()`.
...
הספרייה לא מבצעת retry אוטומטי. timeout לא מלמד אם הבקשה הגיעה לספק — ייתכן שרק התשובה אבדה בדרך
