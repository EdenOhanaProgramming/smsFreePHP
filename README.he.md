...
ולכן כולם עוברים נרמול לצורה קנונית אחת, וכל מה שאינו מספר סלולרי ישראלי תקין מדולג ומדווח, כדי שכל השאר עדיין ייצאו.
...
כברירת מחדל נמען שלא ניתן לפענוח מדולג: ההודעה יוצאת לכל מי שאפשר לפענח, והשאר חוזרים
מ-`SendResult::skippedRecipients()`. שליחה לנמען בודד שלא תקין עדיין נכשלת, כי אין למי לשלוח.
...
אם אתם מעדיפים שמספר אחד לא תקין יפסול את כל הבקשה (שימושי ב-OTP שבו הנמען חייב להיות תקין),
מחליפים את המדיניות:
...
בלבד, נמענים לא תקינים מדולגים, ואימות TLS תמיד דלוק.
...
רישיון MIT מכסה את הקוד, כולל סעיף ה-"as is" שבו: אין אחריות, ואין חבות על מה שקורה בעקבות השימוש.
שני דברים הוא לא מכסה, ושניהם באחריות מי ששולח את ההודעות.

**חוק הספאם.** תיקון 40 לחוק התקשורת (בזק ושידורים) מחייב הסכמה מפורשת מראש לפני שליחת דבר פרסומת,
זיהוי ברור של השולח, ודרך פעילה להסרה. הפיצוי הסטטוטורי מגיע עד 1,000 ש״ח להודעה בלי שהנמען צריך
להוכיח נזק, ולכן דיוור המוני רשלני נהיה יקר מהר מאוד. הודעה תפעולית, כמו קוד אימות או עדכון משלוח
להזמנה שהאדם ביצע, היא עניין שונה מפרסומת.

**התנאים של הספק.** זהו לקוח לא רשמי. החשבון שלכם כפוף לתנאי השימוש של SMS4Free, ושום דבר בחבילה
הזו לא משנה את מה שהם מתירים.

‏smsFreePHP אינו מסונף ל-SMS4Free, אינו מאושר על ידם ואינו קשור אליהם. השם מופיע רק כדי לציין מול
איזה API הספרייה עובדת. אף אחד מהדברים האלה אינו ייעוץ משפטי.
